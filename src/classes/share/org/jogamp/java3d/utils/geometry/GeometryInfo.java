...
import org.jogamp.java3d.internal.ByteBufferWrapper;
import org.jogamp.java3d.internal.ByteOrderWrapper;
import org.jogamp.java3d.internal.FloatBufferWrapper;
...
//EXACT COPY OF j3d org, with IndexRow optomized for it's role as a hashmap member
/**
...
public class GeometryInfo
{
	...
	public void setCoordinates(double coordinates[])
	{
		...
		else
		{
			...
			{
				this.coordinates[i] = new Point3f((float) coordinates[i * 3], (float) coordinates[i * 3 + 1],
	...
	public void setColors(Color3b colors[])
	{
		...
		else
		{
			...
			{
				colors3[i] = new Color3f((float) (colors[i].x & 0xff) / 255.0f, (float) (colors[i].y & 0xff) / 255.0f,
	...
	public void setColors(Color4b colors[])
	{
		...
		else
		{
			...
			for (int i = 0; i < colors.length; i++)
			{
				colors4[i] = new Color4f((float) (colors[i].x & 0xff) / 255.0f, (float) (colors[i].y & 0xff) / 255.0f,
						(float) (colors[i].z & 0xff) / 255.0f, (float) (colors[i].w & 0xff) / 255.0f);
			}
		}
	} // End of setColors
	...
	public void setColors3(float colors[])
	{
		...
		else
		{
			...
			{
				colors3[i] = new Color3f(colors[i * 3], colors[i * 3 + 1], colors[i * 3 + 2]);
			}
		}
	} // End of setColors3
	...
	public void setColors4(float colors[])
	{
		...
		else
		{
			...
			{
				colors4[i] = new Color4f(colors[i * 4], colors[i * 4 + 1], colors[i * 4 + 2], colors[i * 4 + 3]);
			}
		}
	} // End of setColors4
	...
	public void setColors3(byte colors[])
	{
		...
		else
		{
			...
			for (int i = 0; i < colors.length / 3; i++)
			{
				colors3[i] = new Color3f((float) (colors[i * 3] & 0xff) / 255.0f, (float) (colors[i * 3 + 1] & 0xff) / 255.0f,
	...
	public void setColors4(byte colors[])
	{
		...
		else
		{
			...
			for (int i = 0; i < colors.length / 4; i++)
			{
				colors4[i] = new Color4f((float) (colors[i * 4] & 0xff) / 255.0f, (float) (colors[i * 4 + 1] & 0xff) / 255.0f,
						(float) (colors[i * 4 + 2] & 0xff) / 255.0f, (float) (colors[i * 4 + 3] & 0xff) / 255.0f);
			}
		}
	} // End of setColors4
	...
	public void indexify(boolean useCoordIndexOnly)
	{
		...
		if ((DEBUG & 1) == 1)
		{
			...
			for (int i = 0; i < coordinates.length; i++)
			{
				System.out.println("  " + i + " " + coordinates[i] + " " + coordinates[i].hashCode());
			}
	...
	public void compact()
	{
		...
		coordinates = (Point3f[]) compactData(coordinateIndices, coordinates, newInd);
		...
		for (int i = 0; i < texCoordSetCount; i++)
		{
			...
			texCoordSets[i] = compactData(texCoordIndexSets[i], texCoordSets[i], newInd);
	...
	private void checkForBadData()
	{
		...
		if ((coordinateIndices != null) || (colorIndices != null) || (normalIndices != null) || texInds)
		{
			...
			{
				if ((colorIndices != null) || (normalIndices != null) || (texInds == true))
				{
					throw new IllegalArgumentException(J3dUtilsI18N.getString("GeometryInfo20"));
				}
			}
			else if (((colors3 != null) || (colors4 != null)) && (colorIndices == null))
				badData = true;
		...
		{
			if (((colors3 != null) || (colors4 != null)) && (colorIndices.length != coordinateIndices.length))
				badData = true;
			else if ((normals != null) && (normalIndices.length != coordinateIndices.length))
				badData = true;
			else
			{
		...
		//
		if ((prim == TRIANGLE_STRIP_ARRAY) || (prim == TRIANGLE_FAN_ARRAY) || (prim == POLYGON_ARRAY))
		{
			if (stripCounts == null)
				badData = true;
		}
	...
	public void unindexify()
	{
		...
		if (coordinateIndices != null)
		{
			...
			coordinates = (Point3f[]) unindexifyData(coordinates, coordinateIndices);
			coordinateIndices = null;
			...
			for (int i = 0; i < texCoordSetCount; i++)
				texCoordSets[i] = unindexifyData(texCoordSets[i], texCoordIndexSets[i]);
			texCoordIndexSets = new int[texCoordSetCount][];
		}
	} // End of unindexify
	...
	private void fillIn(GeometryArray ga, boolean byRef, boolean interleaved, boolean nio)
	{
		if (interleaved)
		{
			...
			if (nio)
			{
				...
				FloatBufferWrapper f = b.order(ByteOrderWrapper.nativeOrder()).asFloatBuffer();
				f.put(d);
		...
		else if (nio)
		{
			...
			ByteBufferWrapper b = ByteBufferWrapper.allocateDirect(coordinates.length * 4 * 3);
			FloatBufferWrapper f = b.order(ByteOrderWrapper.nativeOrder()).asFloatBuffer();
			...
			for (int i = 0; i < texCoordSetCount; i++)
			{
				b = ByteBufferWrapper.allocateDirect(texCoordSets[i].length * 4 * texCoordDim);
				f = b.order(ByteOrderWrapper.nativeOrder()).asFloatBuffer();
		...
		else if (byRef)
		{
			...
			if (colors3 != null)
				ga.setColorRefFloat(vecmathToFloat(colors3));
			else if (colors4 != null)
				ga.setColorRefFloat(vecmathToFloat(colors4));
			if (normals != null)
				ga.setNormalRefFloat(vecmathToFloat(normals));
			for (int i = 0; i < texCoordSetCount; i++)
			{
		...
		if (coordinateIndices != null)
		{
			...
			{
				if (colorIndices != null)
					iga.setColorIndices(0, colorIndices);
				if (normalIndices != null)
					iga.setNormalIndices(0, normalIndices);
				for (int i = 0; i < texCoordSetCount; i++)
	...
	public GeometryArray getGeometryArray(boolean byRef, boolean interleaved, boolean nio)
	{
		...
		if (nio)
			vertexFormat |= (GeometryArray.BY_REFERENCE | GeometryArray.USE_NIO_BUFFER);
		if (interleaved)
			vertexFormat |= (GeometryArray.BY_REFERENCE | GeometryArray.INTERLEAVED);
		if (byRef)
	 ...
	 * isn't a multiple of three (for triangles) or four (for quads).
	 */
	public IndexedGeometryArray getIndexedGeometryArray(boolean compact, boolean byRef, boolean interleaved, boolean useCoordIndexOnly,
			boolean nio)
	{
		...
		if (useCoordIndexOnly && coordOnly == false)
		{
			...
			if (coordinateIndices != null)
			{
				...
				if (colorIndices != null && colorIndices.length != coordinateIndices.length)
				{
					canUseCoordIndexOnly = false;
				}
				if (normalIndices != null && normalIndices.length != coordinateIndices.length)
				{
					canUseCoordIndexOnly = false;
				}
				for (i = 0; i < texCoordSetCount; i++)
				{
					if (texCoordIndexSets[i] != null && texCoordIndexSets[i].length != coordinateIndices.length)
					{
						canUseCoordIndexOnly = false;
						break;
					}
				}
				if (canUseCoordIndexOnly && ((colorIndices != null) || (normalIndices != null) || (texCoordSetCount > 0)))
				{
					...
					for (i = 0; i < coordinateIndices.length; i++)
					{
						...
						if (colorIndices != null && colorIndices[i] != indexValue)
						{
							canUseCoordIndexOnly = false;
							break;
						}
						if (normalIndices != null && normalIndices[i] != indexValue)
						{
							canUseCoordIndexOnly = false;
							break;
						}
						...
						{
							if (texCoordIndexSets[j] != null && texCoordIndexSets[j][i] != indexValue)
							{
								canUseCoordIndexOnly = false;
								break;
							}
